...
local ingredient_embedding_size = opt.hidden_size
...
local ing_lu = nn.LookupTable(vocab_size, ingredient_embedding_size)
lu.weight = torch.uniform(lu.weight, -0.1, 0.1)
ing_lu.weight = torch.uniform(ing_lu.weight, -0.1, 0.1)
...
if opt.init_dec_from == '' then
    ...
    -- compute the ingredient matrix
    local ingredient_matrix = nn.SelectTable(3)(previous)

    -- attention module. This is the module we'll use to compute attention embeddings
    local function attention_module()
        -- multiply hidden state by ing_matrix, then soft max. (assuming batch size = 4 in comments)
        local ing_matrix_provider, h_provider = nn.Identity()(), nn.Identity()()
        local squeeze1 = nn.Unsqueeze(3)(h_provider) -- (4 x K) => (4 x K x 1)
        local mm = nn.MM()({ ing_matrix_provider, squeeze1}) -- (4 x (N x K)) * (4 x (K x 1)) ==> (4 x (N x 1))
        local squeeze1 = nn.Squeeze()(mm) -- (4 x (N x 1)) ==> (4 x N)
        local softmax = nn.SoftMax()(squeeze1) -- SOFTMAX((4 x N))
        local unsqueeze = nn.Unsqueeze(2)(softmax) -- (4 x N) ==> (4 x (N x 1))
        local mm2 = nn.MM()({unsqueeze, ing_matrix_provider }) --  ((4 x (1 x N)) * (4 x (N x K)) ==> (4 x (1 x K))
        local squeeze2 = nn.Squeeze()(mm2) -- (4 x (1 x K)) ==> (4 x K)
        return nn.gModule({ing_matrix_provider, h_provider}, {squeeze2})
    end

    -- source for hidden state. For i=1, is dec_h0, dec_h0, After is output of i - 1
    local dec_c = nn.SelectTable(1)(previous)
    local dec_h = nn.SelectTable(2)(previous)
    local dropout_dec_lu = nn.SelectTable(4)(previous)
    -- LSTM definition
    local lstm = nn.LSTM(opt.hidden_size + ingredient_embedding_size + opt.wordvec_size, opt.hidden_size)
    lstm.weight = torch.uniform(lstm.weight, -0.1, 0.1)
    lstm.bias = torch.uniform(lstm.bias, -0.1, 0.1)
    lstm.remember_states = true -- absolutely necessary for this to work
    dec_rnns[#dec_rnns + 1] = lstm
    local lstm_nodes = {}
    local combiner = nn.ParallelTable()
    for i=1, opt.max_out_len do
        local selection = nn.Select(2, i)(dropout_dec_lu) -- (4 x T x E) ==> (4 x E)
        local attention = attention_module()({ ingredient_matrix, dec_h})
        local join = nn.Sequential():add(nn.JoinTable(2)):add(nn.Unsqueeze(2))({selection, dec_title_h0, attention}) --input to the LSTM
        local lstm_n
        if i == 1 then
            lstm_n = lstm:clone('weight', 'gradWeight', 'bias', 'gradBias')({dec_c, dec_h, join}):annotate{name = 'lstm_'..i}
        else
            lstm_n = lstm:clone('weight', 'gradWeight', 'bias', 'gradBias')({dec_h, join}):annotate{name = 'lstm_'..i}
        end
        lstm_n = nn.Squeeze()(lstm_n)
        -- collect all the LSTM nodes
        lstm_nodes[#lstm_nodes + 1] = lstm_n
        combiner:add(nn.Unsqueeze(2))
        -- update sources for cell, hidden state
        dec_h = lstm_n
    end

    -- join the hidden states of all the LSTM
    local join = nn.Sequential():add(combiner):add(nn.JoinTable(2))(lstm_nodes) -- {300 x (4 x H)} ==> (4 x 300 x H)
    previous = join
    ...
    -- view and projection to vocab size
    local dec_v1 = nn.View(-1, opt.hidden_size)(previous)
    local dec_lin = nn.Linear(opt.hidden_size, vocab_size)
    ...
    dec = nn.gModule({dec_c0, dec_title_h0, ing_matrix, dec_lu}, {dec_lin}) -- {inputs}, {outputs}
    dec._rnns = dec_rnns
...
-- Creating the ingredient matrix. Lookup table requires matrix (2D) so we flatten and then resize. -1 for title not being present
local make_matrix = nn.Sequential()
make_matrix:add(nn.View(opt.batch_size, -1)):add(ing_lu):add(nn.View(enc_inputs:size(2), enc_inputs:size(3) - 1, enc_inputs:size(4), ingredient_embedding_size)):add(nn.Sum(3)):cuda()
...
local hzeros = torch.CudaTensor.zeros(torch.CudaTensor.new(), opt.batch_size, enc_inputs:size(4)-1, opt.hidden_size)
...
local params, gradParams = combine_all_parameters(enc, dec, make_matrix)
local batch = 1
local epoch = 0
local embs
...
local function print_info(learningRate, iteration, currentError, v_loss, v_perp, t_perp, hidden_states)
    ...
    if opt.save_model_at_epoch then
        ...
        pcall(torch.save, opt.save_prefix..'_mm.th7', make_matrix)
        local log_result
...
local function crossentropy_eval(params)
    ...
    local enc_input = enc_inputs[batch] -- 4 X T
    ...
    local dec_input = dec_inputs[batch]:squeeze() -- 4 X 300
    local output = outputs[batch]:squeeze()
    ...
    -- extract ingredients from encoder input
    ...
    local cell_grad, hidden_grad, ing_grad, dec_in_grad = table.unpack(dec:backward({cb:clone(), dec_h0, ing_matrix, dec_input}, cgrd))

    -- propogate gradients through encoder
    local hidden_grad = torch.reshape(hidden_grad, opt.batch_size, 1, opt.hidden_size)
    local hgrad = torch.cat(hzeros, hidden_grad, 2)
    local egrd = enc:backward(enc_titles, hgrad)

    -- propogate gradients through enc_ingredient matrix embeddings
    make_matrix:backward(enc_ingredients, ing_grad)
...
function get_validation_loss(venc_inputs, vdec_inputs, voutputs, vin_lengths, vout_lengths)
    enc:evaluate()
    dec:evaluate()
    local v_perp, v_loss = attn_perplexity_over_dataset(enc, dec, make_matrix, venc_inputs, vdec_inputs, vin_lengths, vout_lengths, voutputs)
    enc:training()
    dec:training()
