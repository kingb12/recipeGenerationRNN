...
cmd:option('-count_ingredients_used', '')
cmd:option('-beam_decode', false)
cmd:option('-beam_decode_analysis', false)
...
cmd:option('-make_matrix', 'make_matrix.th7')
...
if opt.beam_decode then
    require 'beam_decoder'
end
-- ================================================ EVALUATION =========================================================
...
make_matrix = torch.load(opt.make_matrix)
...
if opt.calculate_perplexity then
    ...
    local tr_perp = attn_perplexity_over_dataset(enc, dec, make_matrix, train_enc_inputs, train_dec_inputs, train_in_lengths, train_out_lengths, train_outputs)
    output['train_perplexity'] = tr_perp
    print('Calculating Valid Perplexity...')
    local vd_perp = attn_perplexity_over_dataset(enc, dec, make_matrix, valid_enc_inputs, valid_dec_inputs, valid_in_lengths, valid_out_lengths, valid_outputs)
    output['valid_perplexity'] = vd_perp
    print('Calculating Test Perplexity...')
    local ts_perp = attn_perplexity_over_dataset(enc, dec, make_matrix, test_enc_inputs, test_dec_inputs, test_in_lengths, test_out_lengths, test_outputs)
...
if opt.generate_samples then
    ...
    output['train_samples'] = attn_generate_samples(train_enc_inputs, train_outputs, opt.num_samples, opt.max_sample_length, gen_inputs, ingredients, opt.beam_decode)
    output['valid_samples'] = attn_generate_samples(valid_enc_inputs, valid_outputs, opt.num_samples, opt.max_sample_length, gen_inputs, ingredients, opt.beam_decode)
    output['test_samples'] = attn_generate_samples(test_enc_inputs, test_outputs, opt.num_samples, opt.max_sample_length, gen_inputs, ingredients, opt.beam_decode)
end
